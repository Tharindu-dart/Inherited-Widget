...
class _InheritedCount extends InheritedWidget {
  ...
  bool updateShouldNotify(_InheritedCount old) {
    print("old = ${old.state.count}");
    print("current = ${state.count}");
    print(old.state.count != state.count);
    return old.state.count!= state.count;   //Always false, because state object newer change.  (its properties can(eg: counter))
  }
}
...
old.state.count <- WRONG
old.state       <- CORRECT

old.state.count != state.count     <- WRONG  (cannot compare properties, only objects)
old.state != state                <- CORRECT
*/
